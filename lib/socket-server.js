...
function computeTransfers(room, ranks) {
  const { betWinner1, betWinner2 } = room.settings;
  const transfers = [];
  const n = ranks.length;
  const add = (from, to, amount) => {
    if (amount > 0 && from && to && from !== to) transfers.push({ from, to, amount });
  };

  // Stuck on 13: the penalty replaces the placings entirely.
  if (room.lastStuck?.length) {
    for (const id of room.lastStuck) add(id, ranks[0], betWinner1 * STUCK_MULTIPLIER);
    return transfers;
  }
...
function maxLossPerRound(settings, playerCount) {
  ...
  return Math.max(placings, betWinner1 * STUCK_MULTIPLIER);
}
...
export function attachGameServer(io) {
  ...
  function handleGameResult(room, result) {
    ...
    if (room.settings.catch2Price > 0 && catcher) {
      room.status = 'catch2';
  ...
  function resolveCatch2(room, didCatch) {
    ...
    if (didCatch) {
      ...
      // banner before the summary appears.
      broadcastRoom(room);
