...
// Winner 1 prize, and with exactly 3 players it pays both prizes.
function maxLossPerRound(settings, playerCount) {
  const { betWinner1, betWinner2 } = settings;
  return playerCount === 3 ? betWinner1 + betWinner2 : Math.max(betWinner1, betWinner2);
}
...
export function attachGameServer(io) {
  ...
  function persistRound(room, net) {
    recordRound({
      roomCode: room.code,
      settings: room.settings,
      players: (room.lastRanks || []).map((id, i) => ({
