...
export const soundBucket = () => process.env.SUPABASE_SOUND_BUCKET || 'sounds';

// Uploads a sound the operator picked and returns its public URL. `upsert`
// replaces the previous file of the same name, so swapping a sound does not
// leave the old one behind.
export async function uploadSound(filename, buffer, contentType) {
  if (!db()) return null;
  const { error } = await db()
    .storage.from(soundBucket())
    .upload(filename, buffer, { contentType, upsert: true });
  if (error) {
    warn('uploadSound', error);
    return null;
  }
  const { data } = db().storage.from(soundBucket()).getPublicUrl(filename);
  // Cache-bust so players hear the new sound rather than the cached old one.
