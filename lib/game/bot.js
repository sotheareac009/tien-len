...
export function generateCombos(hand) {
  ...
  // Double sequences (3+ consecutive pairs, no 2s), with a variant for
...
export function chooseMove(game, playerId) {
  ...
  const beating = combos.filter((c) => beats(c, table));
  ...
  if (isBomb(best) && !isBomb(table) && tableTopRank !== 15) return null;
