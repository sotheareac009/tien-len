...
export class TienLenGame {
  ...
  play(playerId, cardIds) {
    ...
    if (this.hands.get(playerId).length === 0) this.finished.push(playerId);
  ...
  // Used for disconnected players so the round can continue:
  // pass if possible, otherwise lead with the lowest single.
  autoMove(playerId) {
