...
export class TienLenGame {
  constructor(playerIds, riggedHands = null, { starterId = null } = {}) {
    ...
    // Instant wins are decided on the deal, before the opening discard — four
    // 3s would otherwise be swept off the table before anyone saw them.
    this.instantWin = this.detectInstantWin(!winnerLeads);
    if (this.instantWin) {
      this.over = true;
      this.finished = [this.instantWin.playerId];
    }

    this.discarded = [];
    if (!winnerLeads && !this.instantWin) this.discardThrees();
  ...
  // Four 2s or four 3s in one hand wins the round outright, without a card
  // being played. If both are dealt — to one player or to two — the opening
  // round (a new table, or the one after a penalty) is won by the four 3s, and
  // any other round by the four 2s.
  detectInstantWin(openingRound) {
    const holderOf = (rank) =>
      this.order.find((id) => this.hands.get(id).filter((c) => c.rank === rank).length === 4);
    const twos = holderOf(15);
    const threes = holderOf(3);
    if (!twos && !threes) return null;
    if (twos && threes) {
      return openingRound
        ? { playerId: threes, kind: 'four3s' }
        : { playerId: twos, kind: 'four2s' };
    }
    return twos ? { playerId: twos, kind: 'four2s' } : { playerId: threes, kind: 'four3s' };
  }
  ...
  // True when no *other* player is still holding a 2 — the condition that lets
  // a bomb be played on an ordinary single. Your own 2s do not count: if you
  // hold one you can simply play it.
  noTwosLeftFor(playerId) {
    return !this.order.some(
      (id) => id !== playerId && this.hands.get(id)?.some((c) => c.rank === 15)
    );
  }
  ...
  toJSON() {
    return {
      ...
      instantWin: this.instantWin ?? null,
  ...
  static fromJSON(data) {
    ...
    g.instantWin = data.instantWin ?? null;
  ...
  play(playerId, cardIds) {
    ...
    if (this.table && !beats(combo, this.table, { noTwosLeft: this.noTwosLeftFor(playerId) })) {
    ...
    if (this.table) {
      const t = this.table;
      const prevMult = t.chopMult || 1;
      const sameShape = combo.type === t.type && combo.cards.length === t.cards.length;
      const counterSF =
        sameShape && prevMult >= 2 && isStraightFlush(combo) && isStraightFlush(t);
      // Only a 2, a pair of 2s or a bomb can actually be chopped. Taking an
      // ordinary single with a bomb — legal only once the 2s are gone — is
      // just a beat: it wins the trick and earns no bonus.
      const choppable =
        (t.type === 'single' && t.cards[0].rank === 15) ||
        (t.type === 'pair' && t.cards[0].rank === 15) ||
        t.type === 'quad' ||
        (t.type === 'doubleSeq' && t.cards.length >= 8);
      if ((!sameShape && choppable) || counterSF) {
        bomb = { by: playerId, victim: t.playerId, multiplier: prevMult };
        chopMult = prevMult * 2;
      }
    }
    if (!bomb) {
